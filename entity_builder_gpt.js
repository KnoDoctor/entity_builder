...
function createApiFiles(lowercasePlural, params) {
    ...
    writeFile(
        path.join(apiDir, "[identifier].ts"),
        buildApiIdentifierFile(params)
    );
}
...
function createAdminFiles(lowercasePlural, params) {
    ...
    writeFile(
        path.join(adminDir, "[identifier].tsx"),
        buildAdminIdentifierFile(params)
    );
}
...
function createFrontendFiles(lowercasePlural, params) {
    ...
    writeFile(
        ...
        buildEntityIndexFile(params)
    );
    writeFile(
        path.join(frontendDir, "[...identifier].tsx"),
        buildEntityIdentifierFile(params)
    );
}
...
function createHooksFiles(lowercasePlural, params) {
    ...
    writeFile(
        path.join(hooksDir, `use${params.uppercaseSingular}.ts`),
        buildUseEntityFile(params)
    );
    writeFile(
        path.join(hooksDir, `use${params.uppercasePlural}.ts`),
        buildUseEntitiesFile(params)
    );
}

function createOrganismFiles(lowercasePlural, params) {
    const organismDir = path.join(
        baseDir,
        "components",
        "organisms",
        lowercasePlural
    ...
    writeFile(
        path.join(
            organismDir,
            `${params.uppercaseSingular}DeletionOrganism.tsx`
        ),
    ...
    writeFile(
        path.join(
            organismDir,
            `${params.uppercaseSingular}CreationOrganism.tsx`
        ),
